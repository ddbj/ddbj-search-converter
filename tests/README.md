...
- **es/**: index 作成・削除、bulk insert (mock ES)、alias 構成、Blue-Green の swap ロジック、blacklist 削除。**現状は `bulk_insert` / `bulk_delete` / `client` / `index` / `mappings` を整備済。`settings` / `snapshot` / `monitoring` は unit 未整備で integration-scenarios.md (`IT-ES-*`, `IT-BLUEGREEN-*`) でカバー予定。**
