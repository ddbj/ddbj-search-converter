...
`parallel_bulk` / `bulk` の HTTP 失敗を吸収するため、両者とも以下のリトライ設定で動作する。値は `ddbj_search_converter/es/settings.py` の `BULK_MAX_RETRIES` / `BULK_INITIAL_BACKOFF` / `BULK_MAX_BACKOFF` / `BULK_RETRY_ON_STATUS` を SSOT とする。

| 設定 | 値 | 理由 |
|---|---|---|
| `max_retries` | 3 | 一過性のエラー (cluster reroute、shard relocation) は数秒で復旧することが多い |
| `initial_backoff` | 2 秒 | 直後リトライはサーバー側がまだ復旧していない可能性が高い |
| `max_backoff` | 60 秒 | 全体の待ち時間が無制限に伸びないようにする |
