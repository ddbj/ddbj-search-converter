...
#### 公開状態の判定 (manager テーブル)

`accession` / `project` / `link_pr_ac` の 3 テーブルだけでは accession の公開状態を表現できない。INSDC accession の公開状態は `manager` テーブルの `status` カラム (smallint, 7 値) で表現される。採用する `status` は以下の 2 値に限る。

| st_id | st_name | 扱い | 備考 |
|---|---|---|---|
| 1000 | Unknown | 除外 | |
| 1001 | private | 除外 | submission 受理済だが未公開 |
| **1002** | **public** | **採用** | 通常公開済 |
| 1004 | suppressed | 除外 | DDBJ/NCBI 側で表示停止 |
| **1005** | **secondary** | **採用** | 上位 accession に統合された退役 ID |
| 1006 | killed | 除外 | 削除 |
...
| TRAD DB | blacklist 件数 | `set[int]` メモリ目安 |
|---|---:|---:|
| g-actual | ~253K | ~7 MB |
| e-actual | ~1.46M | ~41 MB |
| w-actual | ~8.58M | ~240 MB |

DB ごとに独立した connection / blacklist を持つので同時保持は最大 ~240 MB。converter コンテナのメモリ枠で処理可能。
...
CREATE TABLE dra_fastq_analysis_dir (submission TEXT NOT NULL, analysis TEXT NOT NULL);
CREATE TABLE dra_sra_file (run TEXT NOT NULL);
```

`sync_dra_tar` / `build_dra_tar` の末尾で構築される。JSONL 生成時に DRA エントリーの distribution（FASTQ / SRA / analysis DATA ダウンロードリンク）の有無を判定するために使用。analysis 用テーブルは submission ディレクトリ直下の `DRZ*` サブディレクトリを収集する。
...
| エントリータイプ | JSON | JSON-LD | XML | FASTQ | SRA | DATA |
|-----------------|------|---------|-----|-------|-----|------|
| BioProject | o | o | - | - | - | - |
| BioSample | o | o | - | - | - | - |
| SRA (DRA-origin) 全般 | o | o | o | - | - | - |
| SRA (DRA-origin) sra-run | o | o | o | o\* | o\* | - |
| SRA (DRA-origin) sra-analysis | o | o | o | - | - | o\* |
...
`*` = DRA ファイルインデックス DB にファイル/ディレクトリが存在する場合のみ
`**` = ddbj.nig.ac.jp ミラーへの URL を機械生成 (実在チェックなし)。ミラー欠落により 404 になりうる

GEA の DATA は公開ディレクトリの landing page で、entry 生成元の IDF/SDRF ディレクトリが必ず存在するため実在チェックなしで全 entry に付与する。MetaboBank の DATA も同様に公開ディレクトリの landing page で、entry 生成元の IDF が存在するディレクトリが必ず存在するため実在チェックなしで全 entry に付与する。
...
| DATA (analysis) | `{base}/fastq/{submission[:6]}/{submission}/{analysis}/` | DRA-origin のみ | `dra_fastq_analysis_dir` |
...
FASTQ は他極ミラーが存在しないため、自極 (DRA-origin) でのみ生成する。DATA (analysis) も同様に自極のみで、関連ファイルのミラーリングを行っていないため `DRZ` accession の submission 直下のディレクトリ (`{base}/fastq/{submission[:6]}/{submission}/{analysis}/`) を `dra_fastq_analysis_dir` の実在チェックを経てリンク化する。

GEA の DATA は公開ディレクトリの landing page で、ベース URL は `GEA_PUBLIC_BASE_URL = "https://ddbj.nig.ac.jp/public/ddbj_database/gea"` (`ddbj_search_converter/config.py`)。`{base}/experiment/{prefix}/{accession}/` 形式で、`prefix` は accession 数値部を 1000 単位で切り捨てた `E-GEAD-{NNN}` グループ (FTP 側のディレクトリ構造と一致)。GEA entry は IDF/SDRF が存在するディレクトリからのみ作られ対応ディレクトリが必ず存在するため、実在チェックは行わず全 entry に付与する。

MetaboBank の DATA も公開ディレクトリの landing page で、ベース URL は `METABOBANK_PUBLIC_BASE_URL = "https://ddbj.nig.ac.jp/public/metabobank"` (`ddbj_search_converter/config.py`)。`{base}/study/{accession}/` 形式。MetaboBank entry は IDF が存在するディレクトリからのみ作られ対応ディレクトリが必ず存在するため、実在チェックは行わず全 entry に付与する。
