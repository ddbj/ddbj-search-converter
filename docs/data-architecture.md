...
`accession` / `project` / `link_pr_ac` の 3 テーブルだけでは accession の公開状態を表現できない。INSDC accession の公開状態は `manager` テーブルの `status` カラム (smallint, 7 値) で表現される。converter は `accession` に `manager` を `ac_id` で JOIN し、`manager.status` を以下のように扱う。
