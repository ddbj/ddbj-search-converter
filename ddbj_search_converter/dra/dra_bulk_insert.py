...
from ddbj_search_converter.config import (DRA_JSONL_DIR_NAME, LOGGER, Config,
...
INDEXES = ["sra-submission", "sra-study", "sra-experiment", "sra-run", "sra-sample", "sra-analysis"]
    ...
    def _generate_es_bulk_actions(file: Path, index: str) -> Iterator[Dict[str, Any]]:
        with file.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                if line == "":
                    continue
                doc = SRA.model_validate_json(line)
                yield {
                    "_op_type": "index",
                    "_index": index,
                    "_id": doc.identifier,
...
def parse_args(args: List[str]) -> Tuple[Config, Args]:
    parser = argparse.ArgumentParser(
        description="""\
            Bulk insert JSONL files containing DRA data into Elasticsearch.
...
def main() -> None:
    LOGGER.info("Bulk inserting DRA data into Elasticsearch")
    config, args = parse_args(sys.argv[1:])
    set_logging_level(config.debug)
    LOGGER.debug("Config:\n%s", config.model_dump_json(indent=2))
    LOGGER.debug("Args:\n%s", args.model_dump_json(indent=2))

    latest_dir, prior_dir = get_recent_dirs(config.work_dir.joinpath(DRA_JSONL_DIR_NAME), args.latest_dir, args.prior_dir)
    ...
    LOGGER.info("Finished inserting DRA data into Elasticsearch")
