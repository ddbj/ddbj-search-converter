...
from ddbj_search_converter.config import (BS_JSONL_DIR_NAME, LOGGER, Config,
    ...
    def _generate_es_bulk_actions(file: Path) -> Iterator[Dict[str, Any]]:
        with file.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                if line == "":
                    continue
                doc = BioSample.model_validate_json(line)
                yield {
                    "_op_type": "index",
                    "_index": "biosample",
                    "_id": doc.identifier,
...
def parse_args(args: List[str]) -> Tuple[Config, Args]:
    parser = argparse.ArgumentParser(
        description="""\
            Bulk insert JSONL files containing BioSample data into Elasticsearch.
